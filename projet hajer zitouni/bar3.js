...
d3.csv('table.csv').then(data => {
    ...
    const initialColumn = 'gold';
    updateChart1(initialColumn);
  }).catch(error => {
    console.error('Error loading data:', error);
  });
  
  // Function to update the bar chart based on the selected column
  function updateChart1(column) {
   ...
   // Create a color scale for the bar chart using d3.interpolateTurbo
...
.domain([d3.min(counts), d3.max(counts)]);
    ...
    const bars = svg.selectAll('.bar')
      ...
      .on("mouseover", function(event, d) {
        ...
        d3.select("#tooltip2")
            .style("opacity", 1)
            .html("Nombre des athlètes:"+ (districtCounts[d]))
    ...
    bars.append('title')
       .text(d => `Nombre des athlètes: ${districtCounts[d]}`);
